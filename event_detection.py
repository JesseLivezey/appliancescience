...
def event_detector2(time_ticks, vals):
    ...
    # First check if there is a dumb-simple blip up
    # Works for H1 washer, but breaks many other easy one.
#     if smooth_vals.std() > 100.0:
...
tagged_training_filename = "Tagged_Training_10_23_1350975601.mat"
...
#         L2_Pf_window = L2_Pf[L2_start_index:L2_end_index]
        ...
        if len(confident_candidate_event_intervals) > 0:
            detected_event_interval = np.array([confident_candidate_event_intervals[:,0].min(), confident_candidate_event_intervals[:,1].max()])
        elif len(backup_candidate_event_intervals) > 0:
            detected_event_interval = np.array([backup_candidate_event_intervals[:,0].min(), backup_candidate_event_intervals[:,1].max()])
        else:
